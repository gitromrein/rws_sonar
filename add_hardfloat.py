...
#print("PRE-BUILD (custom): 'add_hardfloat.py'")
...
for source_entry in config.findall("sourceEntries/entry"):
    ...
    excludeD = source_entry.get('excluding')
    if excludeD == None:
        print("None excluding")
    else:
        print("excluding:")
        excludeF = source_entry.get('excluding').split("|")
        print(excludeF)
        for excludeF in excludeF:
            #rws = directory + '/' + excludeF
            exnclude_list.append(directory + '/' + excludeF)
        print(exnclude_list)

   # print(type(excludeF) is str)
...
for cubemx_dir in cubemx_directories:
    ...
    for dirpath, dirnames, filenames in walk(cubemx_dir, followlinks=True):
        for file in filenames:
            if filenames == ('sysmem.c'):
                print("SYSMEM.C out")
            elif file.endswith(('.c', '.cpp', '.s', '.S')):
...
env['SRC_FILTER'] += ['+<System/>']

# Now we add the collected source directories to the src_filter
env['SRC_FILTER'] += src_dirs

#WR List of files excluded from build.  01.02.2034
print(">>>>>>>>>>>>>")
print(exnclude_list)
for exnclude_list in exnclude_list:
    env['SRC_FILTER'] += ['-<' + exnclude_list + '>']
print(">>>>>>>>>>>>>")
